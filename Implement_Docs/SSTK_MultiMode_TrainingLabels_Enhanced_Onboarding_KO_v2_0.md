...
- 2026-06-09 이후 routing_v2 학습 라벨의 최신 canonical 산출물은 **v17 precompute-native run**이다. v14/v16 retrofit 산출물은 설계/회귀 분석 이력으로만 참조한다.
- v17은 v14 positive crop을 복사하지 않고 `feats_c2c3c5_v2_strict_enriched_routed_final.jsonl`, GAIC/CGS/CACNet enriched candidate bank, AP-10K dog/cat pose/head precompute를 입력으로 `training_labels`와 `training_labels_multimode`를 모두 재생성한다.
- GAIC/CGS candidate bank가 없거나 AR bucket이 비어 있으면 label factory 내부에서 synthetic replacement로 대체하지 않는다. 먼저 별도 candidate-bank completion artifact를 생성하고, label factory는 그 완성본을 필수 입력으로 읽는다.
- routing_v2 image-level target은 `image_route_name_no_placement` 6-class다. `placement_intent`는 image-level target에서 제외하고 query/crop mode에만 남긴다.
- crop/query v16 mode는 `route_family_v2 + person_shot_type + placement_intent` 12-class다.
- `food` route와 `person_single_environmental_portrait` class는 제외한다. food/tableware evidence는 `scene`으로 fold하고, environmental은 `context_intent=environmental` auxiliary로만 유지한다.
- pet positive는 AP-10K dog/cat pose/head precompute가 있는 subset에서만 허용한다.
- full_body crop positive는 crop-level lower-body visibility, relaxed support demotion, image-shot consistency gate를 통과해야 한다.
- 기존 multimode crop scoring/explainability의 legacy source of truth는 **v14 구현과 v14 산출물**이다. routing_v2 학습 target의 최신 source of truth는 v17 precompute-native 산출물이다.
...
2026-06-09 이후 routing_v2 학습에는 아래 v17 산출물을 우선 사용한다.

```text
data/SSTK/PhaseA_PhotoPrimary_10K_SigLIP2_v5_20260529/artifacts/training_labels/260609_v17_routing_v2_precompute_native_nofood_petstrict_cropstrict_nms_v5
data/SSTK/PhaseA_PhotoPrimary_10K_SigLIP2_v5_20260529/artifacts/training_labels_multimode/260609_v17_routing_v2_precompute_native_nofood_petstrict_cropstrict_nms_v5
```

세부 run 결과는 아래 문서를 canonical 결과 요약으로 둔다.

```text
data/SSTK/PhaseA_PhotoPrimary_10K_SigLIP2_v5_20260529/artifacts/training_labels_multimode/260609_v17_routing_v2_precompute_native_nofood_petstrict_cropstrict_nms_v5/PRECOMPUTE_NATIVE_V17_RESULTS_KO.md
```
...
# 부록 E. Routing v2 Precompute-Native v17 운영 기준

## E.1 왜 v14/v16 retrofit이 아니라 precompute-native인가

v16 retrofit은 기존 v14 positive crop을 route taxonomy에 맞춰 재해석하는 방식이었다. 이 방식은 빠르게 taxonomy와 visual audit을 만들 수 있지만, crop-level shot type을 원본 precompute와 scorer intent에서 다시 판단하지 않으면 `face` crop이 `full_body`로 오염되는 문제가 생긴다. 실제 audit에서 v16 full-body positive 중 55% 이상이 crop visibility 기준과 충돌했다.

v17은 아래 원칙으로 재생성했다.

- v14 label JSON은 입력으로 쓰지 않는다.
- image-level route는 perception precompute에서 직접 추정하고 positive crop vote로 override하지 않는다.
- crop/query mode는 precompute entity atom, query builder, GAIC/CGS/CACNet candidate bank, AP-10K dog/cat pose/head gate를 통해 새로 연다.
- full_body crop은 crop-level lower-body visibility, relaxed support demotion, image-shot consistency gate를 통과해야 positive가 된다.

## E.2 최신 canonical 산출물

| 항목 | path |
|---|---|
| training_labels | `data/SSTK/PhaseA_PhotoPrimary_10K_SigLIP2_v5_20260529/artifacts/training_labels/260609_v17_routing_v2_precompute_native_nofood_petstrict_cropstrict_nms_v5/` |
| training_labels_multimode | `data/SSTK/PhaseA_PhotoPrimary_10K_SigLIP2_v5_20260529/artifacts/training_labels_multimode/260609_v17_routing_v2_precompute_native_nofood_petstrict_cropstrict_nms_v5/` |
| result brief | `data/SSTK/PhaseA_PhotoPrimary_10K_SigLIP2_v5_20260529/artifacts/training_labels_multimode/260609_v17_routing_v2_precompute_native_nofood_petstrict_cropstrict_nms_v5/PRECOMPUTE_NATIVE_V17_RESULTS_KO.md` |
| validation summary | `data/SSTK/PhaseA_PhotoPrimary_10K_SigLIP2_v5_20260529/artifacts/training_labels_multimode/260609_v17_routing_v2_precompute_native_nofood_petstrict_cropstrict_nms_v5/validation_summary.json` |
| training_labels visualization | `data/SSTK/PhaseA_PhotoPrimary_10K_SigLIP2_v5_20260529/artifacts/training_labels/260609_v17_routing_v2_precompute_native_nofood_petstrict_cropstrict_nms_v5/qualitative_viz/` |
| multimode crop-review panels | `data/SSTK/PhaseA_PhotoPrimary_10K_SigLIP2_v5_20260529/artifacts/training_labels_multimode/260609_v17_routing_v2_precompute_native_nofood_petstrict_cropstrict_nms_v5/crop_review_routing_v2_v17_precompute_native/panels_unified_220/` |
| final audit | `data/SSTK/PhaseA_PhotoPrimary_10K_SigLIP2_v5_20260529/artifacts/training_labels_multimode/260609_v17_routing_v2_precompute_native_nofood_petstrict_cropstrict_nms_v5/crop_review_routing_v2_v17_precompute_native/panels_unified_220/audit_jy_260609_v17/` |

## E.3 Candidate bank 정책

GAIC/CGS/CACNet candidate bank가 없거나 불완전하면 label factory에서 대체 crop을 즉석 생성하지 않는다. 이번 run에서는 원본 candidate bank가 10,000장을 모두 포함했지만 9:16 bucket이 비어 있던 4개 row가 있었다. 따라서 `src/scripts/complete_sstk_candidate_bank_missing_ar.py`로 별도 completion artifact를 먼저 만들었다.

보강 결과:

- completed rows: 4
- added 9:16 candidates: 20
- empty-after AR bucket: 0
- fake teacher score: 없음

## E.4 v17 Taxonomy

Image-level route target은 no-placement 6-class다.

| id | image route |
|---:|---|
| 0 | `scene` |
| 1 | `person_single_face_headshot` |
| 2 | `person_single_upper_half_body` |
| 3 | `person_single_full_body` |
| 4 | `person_group` |
| 5 | `pet_dogcat` |

Crop/query v16 mode는 12-class다.
...
- image route head: no-placement 6-class.
- hierarchical auxiliary head: `route_family_v2`, `person_shot_type`, `context_intent`, `mode_feasible`.
- crop/query mode head: v16 12-class.
- flat full class head: ablation으로만 유지하고 primary decision에는 쓰지 않는다.
...
|---|---:|
| `scene` | 6,084 |
| `person_single_face_headshot` | 473 |
| `person_single_upper_half_body` | 2,136 |
| `person_single_full_body` | 223 |
| `person_group` | 761 |
| `pet_dogcat` | 323 |

Positive crop/query mode 분포:

| mode | positives |
|---|---:|
| `scene_center` | 30,636 |
| `person_single_face_headshot_center` | 7,559 |
| `person_single_upper_half_body_center` | 1,296 |
| `person_single_upper_half_body_rot` | 1,586 |
| `person_single_full_body_center` | 876 |
| `person_single_full_body_rot` | 983 |
| `person_group_center` | 1,302 |
| `person_group_rot` | 691 |
| `pet_dogcat_center` | 1,287 |
| `pet_dogcat_rot` | 1,199 |

## E.6 남은 후속 작업

- `person_group` image route의 single-person suspicious 후보 640건은 별도 group/person decision pack으로 재검수한다. 현재 값은 high-recall 후보이며 모두 오류라는 뜻은 아니다.
- `person_single_face_headshot` image route 중 full-body pose suspicious 195건은 image-level route teacher refinement 후보로 남긴다. v17 v5 crop-level full_body mismatch는 0이지만 image-level shot teacher 자체는 추가 보정 여지가 있다.
- 사람+pet 공동 주피사체 이미지는 현재 primary route 1개만 가진다. 학습에서는 `primary_route + secondary_subject_flags`를 먼저 권장하고, 별도 `person_pet_pair` route class는 precision이 충분한 수동 audit pack 이후 결정한다.
- public teacher score가 없는 candidate completion 후보는 crop quality head의 teacher-score regression target에서 제외하거나 weight를 낮춘다.
