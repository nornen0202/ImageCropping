...
if [ -n "$CURATED_IMAGE_DIR" ]; then
  ...
  echo "Curated Img MP    : $FILTER_CURATED_EXPORT_WORKERS (auto_cap=$FILTER_CURATED_EXPORT_AUTO_CAP, chunk=$FILTER_CURATED_EXPORT_CHUNKSIZE)"
fi
echo "========================================="
...
if [ -n "$CURATED_IMAGE_DIR" ]; then
  EXTRA_ARGS+=(
    ...
    --save_curated_images_workers "$FILTER_CURATED_EXPORT_WORKERS"
    --save_curated_images_auto_workers_cap "$FILTER_CURATED_EXPORT_AUTO_CAP"
    --save_curated_images_chunksize "$FILTER_CURATED_EXPORT_CHUNKSIZE"
  )
fi
