...
PUBLIC_TASK_MANIFEST_JSONL="${PUBLIC_TASK_MANIFEST_JSONL:?public_task_manifest_jsonl required}"
...
payload = {
    ...
    "public_export_log": str(Path(${OUTPUT_DIR@Q}) / "public_benchmark_export.log"),
    "public_eval_log": str(Path(${OUTPUT_DIR@Q}) / "public_benchmark_eval.log"),
    "replay_eval_log": str(Path(${OUTPUT_DIR@Q}) / "replay_eval_test.log"),
    "head_analysis_log": str(Path(${OUTPUT_DIR@Q}) / "head_analysis.log"),
    "direct_val_log": str(Path(${OUTPUT_DIR@Q}) / "direct_val_${DIRECT_SELECTION_POLICY}.log"),
    "direct_test_log": str(Path(${OUTPUT_DIR@Q}) / "direct_test_${DIRECT_SELECTION_POLICY}.log"),
    "direct_top1_log": str(Path(${OUTPUT_DIR@Q}) / "direct_test_utility_top1.log"),
    "summary_json": str(Path(${OUTPUT_DIR@Q}) / "final_eval_bundle_summary.json"),
}
...
EXPORT_PUBLIC_SCRIPT="$(resolve_script export_mobilecropnet_v4_public_benchmark_predictions.py)"
EVAL_PUBLIC_SCRIPT="$(resolve_script evaluate_public_task_manifest_predictions.py)"
...
write_status public_export running
/usr/local/bin/python3 "${EXPORT_PUBLIC_SCRIPT}" \
  --task_manifest_jsonl "${PUBLIC_TASK_MANIFEST_JSONL}" \
  --checkpoint "${CHECKPOINT}" \
  --output_jsonl "${OUTPUT_DIR}/public_benchmark/predictions.jsonl" \
  --summary_json "${OUTPUT_DIR}/public_benchmark/export_summary.json" \
  --method_name "${METHOD_ID}" \
  --project_root "${ROOT}" \
  --device "${DEVICE}" \
  --batch_size "${PUBLIC_BATCH_SIZE}" \
  2>&1 | tee "${OUTPUT_DIR}/public_benchmark_export.log"

write_status public_eval running
/usr/local/bin/python3 "${EVAL_PUBLIC_SCRIPT}" \
  --task_manifest_jsonl "${PUBLIC_TASK_MANIFEST_JSONL}" \
  --predictions_jsonl "${OUTPUT_DIR}/public_benchmark/predictions.jsonl" \
  --output_dir "${OUTPUT_DIR}/public_benchmark/eval" \
  --method_name "${METHOD_ID}" \
  --mode S \
  --score_field score \
  --target_ar_filter_mode hard \
  2>&1 | tee "${OUTPUT_DIR}/public_benchmark_eval.log"
...
(root / "final_eval_bundle_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
print(json.dumps({
    "method_id": summary["method_id"],
    "public_eval_summary": str(root / "public_benchmark" / "eval" / "summary.json"),
    "head_analysis": str(root / "head_analysis" / "head_analysis_summary.json"),
    "direct_test": str(root / "direct_test_${DIRECT_SELECTION_POLICY}" / "metrics.json"),
}, ensure_ascii=False, indent=2))
PY

date -u +"end_utc=%Y-%m-%dT%H:%M:%SZ" | tee -a "${OUTPUT_DIR}/environment.log"
